...
def test_asymm_boolean_values(dataset1):
    """Test that missing values are handled properly
    ...
    Distances from daisy:
        ...
        |           r1        r2        r3        r4 |
        | r1 0.0000000 0.3178266 0.8488400 0.3178266 |
        | r2 0.3178266 0.0000000 0.7705495 0.3627839 |
        | r3 0.8488400 0.7705495 0.0000000 0.8666667 |
        | r4 0.3178266 0.3627839 0.8666667 0.0000000 |
         --------------------------------------------
    """
    dataset1["f"] = [True, False, False, False]
    ...
    gower_dist = gower.gower_distances(
        dataset1,
        feature_types={"f": "asymmbin"},
    )
    target_dist = np.array([
        [0.0000000, 0.3178266, 0.8488400, 0.3178266],
        [0.3178266, 0.0000000, 0.7705495, 0.3627839],
        [0.8488400, 0.7705495, 0.0000000, 0.8666667],
        [0.3178266, 0.3627839, 0.8666667, 0.0000000],
    ])
    np.testing.assert_allclose(gower_dist, target_dist, rtol=1e-05)
